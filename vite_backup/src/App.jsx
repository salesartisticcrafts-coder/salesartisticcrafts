...
function Navbar() {
  ...
  const navItems = [
    { label: 'Shop', sub: ['Marble Jewelry', 'Marble Decor', 'Limited Edition', 'New Arrivals'] },
    { label: 'Collections', sub: ['White Marble', 'Black Marble', 'Onyx', 'Gold & Marble', 'Signature'] },
    { label: 'Custom Studio', sub: [] },
    { label: 'About', sub: ['Our Story', 'Our Materials', 'Sustainability', 'Workshop'] },
    { label: 'Journal', sub: ['Design Inspiration', 'Marble Care Guide', 'Interior Styling'] },
    { label: 'Corporate', sub: [] },
    { label: 'Contact', sub: [] },
  ];
            ...
            <li key={item.label}
              ...
              {item.sub.length > 0 && activeDropdown === item.label && (
                <div className="navbar__dropdown">
                  {item.sub.map(s => (
                    <a key={s} href="#" className="navbar__dropdown-item">{s}</a>
                  ))}
                </div>
              )}
            </li>
...
function Hero() {
  ...
  return (
    <section 
      ...
      id="hero"
      style={{
        '--active-glow': slides[activeSlide].glow
      }}
    >
      {/* 1. Full-Screen Panning Backdrop Marble Texture */}
      <div className="hero__theater-bg-wrap">
        {slides.map((s, idx) => (
          <div 
            key={idx}
            className={`hero__theater-bg ${activeSlide === idx ? 'active' : ''}`}
          >
            <img src={s.img} alt={s.slabName} className="hero__theater-bg-img" />
            <div className="hero__theater-bg-overlay" />
          </div>
        ))}
      </div>

      {/* Background Watermark Typography */}
      <div className="hero__bg-watermark">{slides[activeSlide].watermark}</div>
      ...
      <div className="hero__particles">
        {particles.map(p => (
          <div key={p.id} className="hero__particle"
            style={{ 
              left: `${p.x}%`, 
              top: `${p.y}%`, 
              width: p.size, 
              ...
              transform: `translateY(${scrollTop * (p.size * -0.1)}px)`
            }} />
        ))}
      </div>

      {/* Main Museum Stage Center Viewport */}
      <div className="hero__museum-viewport">
        
        {/* Layered Behind Title */}
        <div key={`title-${activeSlide}`} className="hero__layered-title-container">
          <div className="section-label hero__label animate-slide-up">
            {slides[activeSlide].label}
          </div>
          <h1 className="hero__editorial-title animate-slide-up">
            <span className="title-wrapper">
              <span className="title-line">{slides[activeSlide].titleLine1}</span>
            </span>
            <span className="title-wrapper">
              <span className="title-line title-italic">{slides[activeSlide].titleItalic1}</span>
            </span>
            <span className="title-wrapper">
              <span className="title-line">{slides[activeSlide].titleLine2}</span>
            </span>
            <span className="title-wrapper">
              <span className="title-line title-italic">{slides[activeSlide].titleItalic2}</span>
            </span>
          </h1>
        </div>
        ...
        <div 
          key={`stage-${activeSlide}`}
          className="hero__museum-stage"
        >
          <div 
            className="museum-stage__interactive-wrap"
            ref={cardRef}
          ...
          >
            ...
            <div className="museum-aura" />

            {/* The Glass Plinth Base */}
            <div className="museum-plinth">
              <div className="museum-plinth__rim" />
              <div className="museum-plinth__surface" />
            </div>

            {/* Floating Product Image */}
            <div className="museum-product-container">
              <img 
                src={slides[activeSlide].img} 
                alt={slides[activeSlide].label} 
                className="museum-product-img" 
              />
              <div className="hero__arch-glint" />
            </div>

            {/* Magnifier glass inside the Arch */}
            {magnifier.show && (
              <div 
                className="hero__arch-magnifier" 
                style={{
                  left: `${magnifier.x}px`,
                  top: `${magnifier.y}px`,
              ...
              >
                {/* Precision UI grid in loupe to simulate magnifying tool */}
                <div className="loupe-reticle-h" />
                <div className="loupe-reticle-v" />
                <span className="loupe-badge">ATELIER LOUPE 2.5X</span>
              </div>
            )}
          </div>
        </div>

        {/* Floating Left: Technical specifications Blueprint */}
        <div key={`specs-${activeSlide}`} className="hero__float-card hero__float-card--left animate-slide-up">
          <div className="float-card__header">GEOLOGICAL SPECIFICATIONS</div>
          <div className="float-card__specs">
            <div className="float-spec">
              <span className="label">Origin</span>
              <span className="value">{slides[activeSlide].specs.origin}</span>
            </div>
            <div className="float-spec">
              <span className="label">Veining</span>
              <span className="value">{slides[activeSlide].specs.veining}</span>
            </div>
            <div className="float-spec">
              <span className="label">Mohs Hardness</span>
              <div className="value-bar">
                <span className="val-text">{slides[activeSlide].specs.hardness}</span>
                <div className="bar"><div className="fill" style={{ width: `${slides[activeSlide].specs.hardnessLevel}%` }} /></div>
              </div>
            </div>
          </div>
        </div>

        {/* Floating Right: Quarry Coordinates & Rarity */}
        <div key={`coords-${activeSlide}`} className="hero__float-card hero__float-card--right animate-slide-up">
          <div className="float-card__header">QUARRY ACQUISITION INFO</div>
          <div className="float-card__specs">
            <div className="float-spec">
              <span className="label">Coordinates</span>
              <span className="value coords-text"><span className="coords-icon">◈</span> {slides[activeSlide].coords}</span>
            </div>
            <div className="float-spec">
              <span className="label">Status</span>
              <span className="value rarity-badge">{slides[activeSlide].specs.rarity}</span>
            </div>
          </div>
        </div>

        {/* Floating Center Bottom: Subtitle and CTAs */}
        <div key={`info-${activeSlide}`} className="hero__editorial-info animate-slide-up">
          <p className="hero__editorial-subtitle">
            {slides[activeSlide].subtitle}
          </p>
          <div className="hero__actions">
            <a href={slides[activeSlide].ctaLink} className="btn-primary">
              <span>{slides[activeSlide].ctaText}</span> 
              <ArrowRight size={16} />
            </a>
            <a href="#custom" className="btn-ghost">
      ...
      {/* Spanned Atmosphere Selector (The 3 Texture Slabs) */}
      <div className="container hero__selector-container">
        <div className="hero__atmosphere-selector">
          <span className="selector-title">SELECT MATERIAL ATMOSPHERE</span>
          <div className="slab-grid">
            {slides.map((s, idx) => (
              <button 
                key={s.slabName}
                className={`slab-card ${activeSlide === idx ? 'active' : ''}`}
                onClick={() => {
                  setActiveSlide(idx);
                  setTilt({ x: 0, y: 0 });
                  setMagnifier(prev => ({ ...prev, show: false }));
              ...
              >
                <div className={`slab-texture slab-texture--${s.colorName}`}>
                  {activeSlide === idx && (
                    <svg className="slab-timer-svg" viewBox="0 0 36 36">
                      <circle className="slab-timer-bg" cx="18" cy="18" r="16" />
                      <circle className="slab-timer-fill" cx="18" cy="18" r="16" />
                    </svg>
                  )}
                </div>
                <div className="slab-info">
                  <span className="slab-number">0{idx + 1}</span>
                  <div className="slab-meta">
                    <span className="slab-title">{s.slabName}</span>
                    <span className="slab-sub">{s.slabOrigin}</span>
                  </div>
                </div>
              </button>
            ))}
...
function BestSellers() {
  ...
  const products = [
    { id: 1, name: 'Carrara Signet Ring', price: '₹8,500', img: '/images/jewelry.png', tag: 'Best Seller', rating: 5 },
    { id: 2, name: 'Onyx Pendant Noir', price: '₹12,200', img: '/images/black_marble.png', tag: 'New', rating: 5 },
    { id: 3, name: 'White Marble Vase', price: '₹15,800', img: '/images/decor.png', tag: 'Limited', rating: 5 },
    { id: 4, name: 'Calacatta Drop Earrings', price: '₹6,400', img: '/images/jewelry.png', tag: 'Trending', rating: 4 },
    { id: 5, name: 'Nero Sculpt Candle', price: '₹9,900', img: '/images/decor.png', tag: 'Staff Pick', rating: 5 },
    { id: 6, name: 'Gold Marble Bracelet', price: '₹18,500', img: '/images/jewelry.png', tag: 'Exclusive', rating: 5 },
  ];
      ...
      <div className="container">
        ...
        <div className="testimonials__slider">
          {testimonials.map((t, i) => (
            <div key={i} className={`testimonial-card ${i === active ? 'active' : ''}`}>
              <div className="testimonial-card__stars">
                {Array.from({ length: t.stars }).map((_, j) => (
                  <Star key={j} size={16} fill="#C9A96E" stroke="none" className="testimonial-star" style={{ animationDelay: `${j * 0.1}s` }} />
                ))}
              </div>
        ...
        <div className="testimonials__dots">
          {testimonials.map((_, i) => (
            <button key={i} className={`testimonials__dot ${i === active ? 'active' : ''}`} onClick={() => setActive(i)} aria-label={`Testimonial ${i + 1}`} />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <section className="newsletter" id="newsletter">
      <div className="newsletter__bg">
        <img src="/images/marble_hero.png" alt="" className="newsletter__bg-img" />
        <div className="newsletter__bg-overlay" />
      </div>
      <div className="container">
        <div className="newsletter__content reveal">
          <div className="section-label" style={{ color: 'var(--gold-light)' }}>Join The Inner Circle</div>
          <div className="gold-line center" />
          <h2 className="newsletter__title">Be the First to Discover<br />New Arrivals & Exclusive Offers</h2>
          <p className="newsletter__desc">Receive private previews of new collections, invitations to exclusive events, and marble care tips from our master artisans.</p>
          <form className="newsletter__form" onSubmit={e => e.preventDefault()}>
            <input type="email" placeholder="Your email address" className="newsletter__input" required />
            <button type="submit" className="btn-primary newsletter__btn"><span>Subscribe</span> <ArrowRight size={16} /></button>
          </form>
          <p className="newsletter__fine">We respect your privacy. Unsubscribe at any time.</p>
        </div>
      </div>
    </section>
  );
        ...
        <div className="container">
          <div className="footer__grid">
            ...
            <div className="footer__col">
              ...
              {['Marble Jewelry', 'Marble Decor', 'Limited Edition', 'New Arrivals', 'Corporate Gifting'].map(l => (
            ...
            <div className="footer__col">
              ...
              {['Our Story', 'Our Materials', 'Sustainability', 'Workshop', 'Journal', 'Careers'].map(l => (
                <a key={l} href="#" className="footer__link">{l}</a>
              ))}
            </div>
            {/* Contact */}
